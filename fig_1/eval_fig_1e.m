...
shVals=zeros(nSamp,3);
shTrend=zeros(nSamp,2,length(spineVec));
parfor iRep=1:nSamp
    test=0
    while test==0
        try
            nAll=0;
            [outstruct] = genTree_shV(lA,lD);
            shVals(iRep,:)=[outstruct.shVol , outstruct.Ld , outstruct.La];
            ...
            for iSpine=1:length(spineVec)
                ...
                nSyn=size(csyns,1);
                iTrend(1,iSpine)=outstruct.shVol;
...
savename='../data/fig_1/volume_panel.mat';
save(savename,'shTrend','shVals')
clear